...
// Article with reveal-on-scroll wrapping
const FeaturedArticle = ({
  item,
  idx,
  t,
}: {
  item: ReturnType<typeof useHeritages>["data"] extends (infer U)[] | undefined ? U : never;
  idx: number;
  ...
  return (
    <article
      ...
      className={`group grid md:grid-cols-12 gap-8 items-stretch ${
        idx % 2 === 1 ? "md:[&>a:first-child]:order-2" : ""
      }`}
    >
      <Link
        to={`/di-san/${item.slug}`}
        className="md:col-span-5 relative overflow-hidden aspect-[4/5] bg-patina block heritage-img-wrap shadow-bronze"
      ...
      >
        <img
          ...
          alt={item.title}
          loading="lazy"
          className="absolute inset-0 w-full h-full object-cover animate-ken-burns group-hover:scale-110 group-hover:brightness-110"
          style={{ animationDelay: `${idx * -4}s` }}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-patina-deep/85 via-patina-deep/10 to-transparent" />
        ...
        <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
          ...
          <span className="text-[10px] tracking-[0.3em] uppercase text-gold opacity-0 translate-y-1 group-hover:opacity-100 group-hover:translate-y-0 transition-all flex items-center gap-1">
        ...
        <div className="relative pl-6 border-l border-gold/30 space-y-5">
          ...
          {item.history.slice(0, 3).map((h, i) => (
            <div key={i} className="relative">
              ...
              <div className="font-serif-vn italic text-gold-light text-lg mb-1">{h.era}</div>
              <p className="text-background/80 text-sm leading-relaxed max-w-xl">{h.note}</p>
            </div>
          ))}
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="space-y-20">
          {featured.map((item, idx) => (
            <FeaturedArticle key={item.slug} item={item} idx={idx} t={t} />
          ))}
        </div>
        ...
        <div className="mt-24 pt-16 border-t border-gold/15">
          ...
          <div className="flex flex-wrap justify-center gap-3 max-w-5xl mx-auto">
            {others.map((h, i) => (
              <RevealChip key={h.slug} delay={((i % 3) + 1) as 1 | 2 | 3}>
                <Link
                  to={`/di-san/${h.slug}`}
                  className="group text-sm px-4 py-2 border border-gold/30 text-background/80 hover:border-gold hover:text-gold hover:bg-gold/5 hover:-translate-y-0.5 hover:shadow-[0_10px_30px_-15px_hsl(var(--gold)/0.6)] transition-all duration-500 flex items-center gap-2"
                >
                  {h.title}
